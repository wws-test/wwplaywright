...
def test_set_management1( login_goto_pro: PageWrapper):
    navigate_to_solenoid_diagnosis_page(login_goto_pro)
    login_goto_pro.get_by_text("控制系统",exact=True).click()
    login_goto_pro.get_by_text("报警列表",exact=True).click_with_timing()


@allure.story("驾驶舱项目-控制系统-预测性维护")
def test_predictive_maintenance1( login_goto_pro: PageWrapper):
    navigate_to_solenoid_diagnosis_page(login_goto_pro)
    login_goto_pro.get_by_text("控制系统").click()
    login_goto_pro.get_by_text("预测性维护").click_with_timing()


@allure.story("驾驶舱项目-阀门-调节阀预测")
def test_valve_adjust_predict1(login_goto_pro: PageWrapper):
...
@allure.story("现存报警")
def test_workshop3_monitoring1( login_goto_pro,):
    page = navigate_to_solenoid_diagnosis_page1(login_goto_pro)
    page.get_by_text("报警", exact=True).click()
    page.get_by_text("现存报警").click_with_timing()


@allure.story("历史报警")
def test_workshop4_monitoring1( login_goto_pro,):
    page = navigate_to_solenoid_diagnosis_page1(login_goto_pro)
    page.get_by_text("报警", exact=True).click()
...
@allure.story("开停工监测")
def test_workshop_monitoring1(login_goto_pro,):
    page = navigate_to_solenoid_diagnosis_page1(login_goto_pro)
    page.get_by_text("开停工监测").click_with_timing()


@allure.story("冗余仪表偏差报警")
def test_workshop1_monitoring1(login_goto_pro,):
    page = navigate_to_solenoid_diagnosis_page1(login_goto_pro)
    page.get_by_text("冗余仪表偏差报警").click_with_timing()


@allure.story("设备台账")
def test_workshop2_monitoring1( login_goto_pro,):
    page = navigate_to_solenoid_diagnosis_page1(login_goto_pro)
