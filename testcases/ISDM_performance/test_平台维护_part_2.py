...
@allure.story("数据源管理")
def test_datamanager_management1(login_goto_pro):
    page = navigate_to_solenoid_diagnosis_page(login_goto_pro)
    page.locator("#menu").get_by_text("位号配置").click()
...
def test_idm_diagnosis_item_configuration1(login_goto_pro):
    page = navigate_to_solenoid_diagnosis_page(login_goto_pro)
...
@allure.story("联锁回路管理")
def test_self_control_circuit_management1(login_goto_pro):
    page = navigate_to_solenoid_diagnosis_page(login_goto_pro)
...
def test_teardown_module(login_goto_pro):
    ...
    login_goto_pro.locator("//div[@id='root']/div[1]/section[1]/header[1]/div[1]/div[1]").click()
