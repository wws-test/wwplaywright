...
from pages.调节阀 import  *

def navigate_to_solenoid_diagnosis_page(login_goto_project):
    """导航到电磁阀在线诊断页面，并返回框架定位符"""
    with allure.step("导航到调节阀在线诊断页面"):
        login_goto_project.page.locator("span[title='调节阀诊断']").click()
        login_goto_project.page.locator("span[title='调节阀在线诊断']").click()
        frame_locator = login_goto_project.page.frame_locator("iframe[name=\"supos-tab-framework-1\"]")
        frame_locator.locator("//span[normalize-space(text())='调节阀在线诊断']").click() # 点击返回按钮
    return frame_locator


@allure.feature("调节阀在线诊断")
@allure.step("调节阀在线诊断-查询导出")
...
@allure.feature("调节阀在线诊断")
@allure.step("调节阀在线诊断-详情页面")
def test_detail_page( pytestconfig, login_goto_project):
    frame_locator = navigate_to_solenoid_diagnosis_page(login_goto_project)
    with allure.step("点击第二个详情"):
        logger.info("点击详情")
        frame_locator.locator("(//div[@class='model-pages-home-table-index-table_operate']//a)[3]").click()
