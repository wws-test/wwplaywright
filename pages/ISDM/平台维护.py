...
def add_self_control_circuit_management_p(frame_content: PageWrapper):
    ...
    frame_content.get_by_title("wwtest").nth(1).click()
    frame_content.locator(".ace_content").click()
    frame_content.locator("textarea").nth(1).fill("ret\n\n")
    frame_content.get_by_role("button", name="确 定").click()
    frame_content.get_by_label("联锁名称").click()
    frame_content.get_by_label("联锁名称").fill("ww333")
...
def bit_number_configuration_p(frame: PageWrapper):
    ...
    frame.get_by_title("maoming").click()
...
def set_data_source_p(frame: PageWrapper):
    ...
    frame.get_by_placeholder("请输入").first.fill("wws")
    frame.get_by_role("button", name="search 搜索").click()
...
def valve_alarm_configuration_p(frame_locator: PageWrapper):
    ...
    frame_locator.get_by_label("设备").fill("wwtest")
    frame_locator.get_by_title("wwtest", exact=True).locator("div").click()
    frame_locator.get_by_label("报警方式").click()
    frame_locator.get_by_text("主动").click()
    frame_locator.get_by_label("报警类别").click()
    frame_locator.locator(
        "div:nth-child(5) > div >.rc-virtual-list >.rc-virtual-list-holder > div >.rc-virtual-list-holder-inner > div:nth-child(2) >.ant-select-item-option-content").click()
    frame_locator.get_by_label("监测值位号").click()
    frame_locator.get_by_label("监测值位号").fill("ww")
    frame_locator.get_by_title("VIRTUAL-POINT@wwtest1").locator("div").click()
    frame_locator.locator("#monitoringValue").click()
    frame_locator.locator("#monitoringValue").fill("1")
